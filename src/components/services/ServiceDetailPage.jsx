...
const ServiceDetailPage = () => {
    ...
    if (service) {
      const transformedDescriptions =
        service.descriptions?.map((desc) => ({
          ...
          points: desc.points?.map((point) => point.content) || [],
        })) || [];
  ...
  const handleUpdateSubmit = async (e) => {
    ...
    try {
      ...
      const transformedMethods = serviceForm.methods.map((method) => ({
        name: method,
      }));
      ...
      >
        ...
        >
          <GlassCard sx={{ p: 6, maxWidth: 500, textAlign: "center" }}>
            ...
            >
              <WarningIcon
                sx={{ fontSize: 64, color: "warning.main", mb: 3 }}
              />
            </motion.div>
            <Typography variant="h4" fontWeight="bold" gutterBottom>
              Service Not Found
    ...
    >
      ...
      <HeroSection>
        <motion.div style={{ scale: heroScale }}>
          <HeroImage src={mainImage} alt={service.title} />
        </motion.div>
                        ...
                        <List>
                          {description.points.map((point, pointIndex) => (
                            <ListItem key={pointIndex}>
                              <ListItemText primary={point.content} />
                            </ListItem>
                          ))}
                        </List>
                      )}
              ...
              <Fab
                ...
                sx={{
                  ...
                  "&:hover": {
                    transform: "scale(1.1)",
                    background:
                      "linear-gradient(135deg, #5a67d8 0%, #6b4d91 100%)",
                  },
                }}
              >
                <ArrowUpIcon />
